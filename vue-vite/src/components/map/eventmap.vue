<template>
    <div class="home-head">
       ...
       <el-button @click="btnout" class="home-head-out">返回</el-button>
    ...
    AMapLoader.load({
            ...
            plugins:[''],       // 需要使用的的插件列表，如比例尺'AMap.Scale'等
             ...
             map = new AMap.Map("container",{  //设置地图容器id
                ...
                zoom:16,    //初始化地图级别
                terrain: true,       
                center:[props.event.eventLongitude,props.event.eventLatitude], //初始化地图中心点位置
             });
            // 获取点击的经纬度
...
const MarkerPoint=async (point)=>{
    ...
    // 撒点点击事件
    marker.on("click",()=>{
        MarkerClick(point.lng,point.lat)
    })
    map.add(marker);
}
// 点击撒点
...
const point=reactive({
    ...
    lng:props.event.eventLongitude,
    lat:props.event.eventLatitude,
    data:props.event
})
setTimeout(()=>{
    MarkerPoint(point)
